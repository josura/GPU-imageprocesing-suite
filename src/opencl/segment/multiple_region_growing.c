...
cl_event region_growing(cl_kernel region_growing_k, cl_command_queue que,
	cl_mem d_region_labels, cl_mem d_explore_labels, cl_mem d_input, float dist_threshold,
...
void usage(int argc){
	if(argc<4){
		fprintf(stderr,"Usage: ./multiple_region_growing <image.png> num_regions <output.png> [dist_threshold]\n ");
...
int main(int argc, char ** args){
	...
	if(argc > 4) dist_threshold = atof(args[4]);
