...
void usage(int argc){
	if(argc<5){
		fprintf(stderr,"Usage: ./erosion <image.png> <strel.png> <output.png> <mask.png>\n ");
        ...
        fprintf(stderr,"mask is the image used for geodesic erosion\n");
...
int main(int argc, char ** args){
	...
	unsigned char * imgmask= stbi_load(args[4],&maskwidth,&maskheight,&maskchannels,STBI_rgb_alpha);
    ...
    //GEODESIC EROSION TEST
	unsigned char * processedGE= fullGeodesicErosion(img,imgstrel,imgmask,10,width,height,channels,maskwidth,maskheight,maskchannels,strelwidth,strelheight,strelchannels);
    if(processedGE==NULL){
        fprintf(stderr,"problems in method for geodesic erosion\n");
        exit(1);
    }

    //GEODESIC DILATION TEST
	unsigned char * processedGD= fullGeodesicDilation(img,imgstrel,imgmask,10,width,height,channels,maskwidth,maskheight,maskchannels,strelwidth,strelheight,strelchannels);
    if(processedGD==NULL){
