...
cl_event dithering(cl_kernel dithering_k, cl_command_queue que,
	...
	cl_int matrix_dim, cl_int nrows, cl_int ncols, cl_uchar num_levels)
{
	...
	ocl_check(err, "set dithering arg %d", i-1);
	err = clSetKernelArg(dithering_k, i++, sizeof(matrix_dim), &matrix_dim);
	ocl_check(err, "set dithering arg %d", i-1);
...
void usage(int argc){
	if(argc<4){
		fprintf(stderr,"Usage: ./ordered_dithering <image.png> num_levels matrix_dim <output.png>\n ");
...
int main(int argc, char ** args){
	...
	cl_kernel dithering_k = clCreateKernel(prog, "ordered_dithering", &err);
	ocl_check(err, "create kernel ordered_dithering");
	...
	cl_mem d_matrix = NULL, d_input = NULL, d_output = NULL;
	...
	cl_event matrixgen_evt, dithering_evt, map_evt;

	dithering_evt = dithering(dithering_k, que, d_output, d_input, matrix_dim, height, width, num_levels);
	...
	printf("ordered dithering: %dx%d int in %gms: %g GB/s %g GE/s\n",
